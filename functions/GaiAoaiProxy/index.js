...
module.exports = async function (context, req) {
  try {
    ...
    const systemPrompt = process.env.SYSTEM_PROMPT || 'あなたは用語集の説明を行うアシスタントです。出力は必ず日本語の平文のみで、Markdown、箇条書き、装飾記号は使用しないでください。対象はIT用語（ソフトウェア、ハードウェア、ネットワーク、データベース、セキュリティ、クラウド、AI、プログラミング、開発運用など）に限定します。入力がIT用語でない場合は「この用語はIT用語ではないため登録できません」とだけ返してください。IT用語の場合は事実ベースで簡潔に、一言サマリの1文だけを返してください。余計な前置きや追加説明、例、箇条書き、見出しは一切出力しないでください。';
